...
## DA-engine scope amendment: the headline needs a second-moment-matched imputer (2026-07-02, Marcus ratified, D-22)

A cross-model audit (Claude Fable 5, max effort, `todo/040-audit-response-fable5.md`, commit `cc62ad7`; independently confirmed by GPT-5.5 Pro blind + red-team at xhigh effort, `todo/040b-gpt55-gate-package.md`, gate responses at `verification/cache/layer3-gpt-5.5-pro-da-engine-mode{A,B}.md`), triggered by the v1 review's data-augmentation gap (r1#8), found that `derivation.qmd`'s estimated-scale $+\tfrac12\operatorname{tr}(\mathrm{RIV})$ headline as stated ("deterministic FIML and proper MI alike", the old line 423) over-generalizes: a genuine Bayesian posterior draw is over-dispersed relative to the fitted plug-in by construction, and the deviance -- a sum of $n$ per-cell $O(1/n)$-sensitive terms -- resolves that over-dispersion into an $O(1)$ correction. The headline is now scoped to *second-moment-matched* imputers, for a *regular* draw law: $\Xi=b_\Sigma+\operatorname{Var}(\mu^\ast)=o(1/n)$ per cell **and** the design-imbalance term controlled -- deterministic FIML trivially, and bootstrap resampling with an EM plug-in (Amelia's EMB, this paper's own validated engine) via an exact complete-data second-moment identity. A genuine posterior draw carries the disclosed, computable correction $D=-\tfrac12\operatorname{tr}(\mathrm{RIV}_{\mu\mu})-\tfrac12\sum_i\operatorname{tr}(\hat V_i^{-1}C_ib_\Sigma C_i^\top)-\mathrm{IMB}$. The joint (not bare) $\Xi$-and-imbalance condition, and the regularity qualifier, were added after GPT-5.5 Pro's red-team pass constructed valid counterexamples against the bare "$\Xi=o(1/n)$, iff" form Fable's audit had proposed. This paper's own W1/W3 simulations are unaffected (Amelia's EMB is second-moment-matched); the amendment narrows the scope statement and adds a disclosed correction term for the engines it excludes, not a change to any reported number. **Superseding note:** the cross-model triage also found that Fable's "83% centering bias / 13% mu-wander / ~4% imbalance-plus-transient" percentage decomposition of the round-1 scalar-deflated-arm diagnostic (`-0.244`) does not hold up -- the deflation acts inside an iterative data-augmentation P-step, not as a one-shot rescale, and the true mechanism is a DA feedback effect, not linear rescaling. The `-0.244`/`-0.78` numbers themselves are unaffected and independently reconfirmed at higher precision; only that percentage story is retracted, and it was never cited in the landed text below. Verification: four algebraic lemmas checked symbolically (`verification/cas-wolfram/verify_engine_gap_sympy.py`, 10/10 PASS, incl. a closed-form $p=1$ toy table -- Jeffreys $-1$, Laplace $-1/4$, bootstrap $0$ -- that matched the GPT-5.5 gate's own withheld grading key exactly); the numerical decomposition promoted to a cached module (`verification/_modules/engine-gap-audit.R`, not yet wired into `run_all.R`). Landed in `derivation.qmd` at D9 (new forward-pointer, :391-394), the fitted-vs-oracle callout (the "FIML and proper MI alike" sentence corrected + a new scope-and-correction callout inserted, :422-479), and the known-scale callout (one added sentence, :812-816). Full staging record with the gate-triage detail: `todo/040-derivation-amendment-draft.md`.
