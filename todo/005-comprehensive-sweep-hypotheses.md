...
**Sign-off:** Signed by marcus.waldman, 2026-05-23. Predictions in §1 are frozen; sweep executes against this document.
...
- 2026-05-23 — Signed off by marcus.waldman. Sweep execution per todo/006 begins.
- 2026-05-23 — **H3b scope cut**: the rate=60% arm of H3b (3 cells: N ∈ {200, 500, 1000} at rate=60% non_monotone, M=200, congenial Amelia) was deferred to a follow-up rather than executed. Reason: at rate=60% non_monotone, the mice::ampute target translates to 90% incomplete-case prevalence, which causes Amelia's bootstrap to land on resamples where σ_{12} is unidentifiable and Armadillo's `chol()` throws a C++ exception that bypasses R-level tryCatch and kills the PSOCK worker. Recovery options were (a) add a small empri stability prior to make Amelia robust at high rate — rejected because it amends "congenial" mid-preregistration; (b) lower the high rate to what Amelia tolerates — rejected because it weakens the "linear scaling" test; (c) defer the high-rate arm — chosen. H3b now executes as a **low-rate arm only** (3 cells: N ∈ {200, 500, 1000} at rate=20%), and the 40%-rate data from the main W3 sweep (MAR + non_monotone + M=200 + congenial Amelia) provides a second rate point for partial monotone-in-rate testing across rates {20%, 40%}. H3b's "ratio approx 3" criterion is downgraded to observational; the monotone-in-rate test still applies across the 2 available rate points. Reporting commitment: §5 of the manuscript will note H3b ran as a partial preliminary; the full rate-sensitivity sweep is named as future work.
- 2026-05-23 — **Sweep complete.** Verdicts: H1 FAIL (direction reversed; pre-flight predicted), H2-MAR PASS, H2-MNAR observational with no breakdown, H3 PASS, H3b observational with super-linear rate scaling (ratio 3.11 vs predicted 2.0), H4 PASS, H5 observational with partial halving but W3-C survives, H6 PASS. Quantitative: 4 pass, 1 fail, 3 observational. Headline finding: **W3-C = 1.000 across all 60 main sweep cells + 3 rate cells + every congeniality and engine variant** — perfect directional concordance, no exception. See IDEAS.md "Comprehensive sweep results (2026-05-23)" for the full landing and §5/§6 implications. Summary CSVs at `verification/cache/W3-sweep-summary.csv`, `verification/cache/W3-sweep-prod/summary.csv`, `verification/cache/W1-sweep-prod/summary.csv`, `verification/cache/W3-sweep-rate-prod/summary.csv`.
- 2026-05-23 — **Refactor + rerun.** User flagged script sprawl (8 standalone files) and reproducibility concern. Refactored into single-entrypoint `verification/run_all.R` with `_modules/` (smoke-tests, w1-sweep, w3-sweep, w3-rate, mnar-bias, aggregate) and `_legacy/` for retired scripts. Added **Phase 4 (MNAR parameter-bias measurement)** as a permanent phase to directly quantify `E[theta_obs - theta_truth]` under MAR / MNAR / MNAR_targeted — answers the calibration question "did MNAR actually bias the parameters?" Also added `MNAR_targeted` as a third mechanism in `apply_missingness_ampute()` with weights designed to disrupt the σ_13/σ_24 joint distributions; pilot at R=200 showed targeted weights do NOT amplify load-bearing parameter bias beyond standard MNAR (within ampute's linear weight regime, ~13% σ_24 bias is the engineering ceiling). Rerun executed `Rscript verification/run_all.R 2000 prod all 20` end-to-end in 183 min, zero per-replicate errors across all 5 phases. Verdicts identical to prior run (same headline: W3-C = 1.000 universal). Phase 4 measurement reveals: MAR `‖bias‖` decays as 1/√N (~0.003 at N=1000); MNAR `‖bias‖` is N-invariant at 0.23–0.29 (relative 11–14%); σ_24 biased -13% of truth under monotone MNAR (|z|=56); W3-C survives because the candidate-model comparison is robust to bias on μ and diagonal variances (which carry most of the MNAR perturbation) and modestly tolerant of σ_24 bias up to ~13% in this design. Refined H2-MNAR claim now grounded in direct empirical evidence.
