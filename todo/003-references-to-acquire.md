...
| Proposed citation | Status | Claim it supports | Where it would land | Priority | Notes |
|---|---|---|---|---|---|
| **Honaker, J., King, G. & Blackwell, M. (2011).** "Amelia II: A Program for Missing Data." *JSS* 45(7). | **queued** (PDF in Zotero 2026-06-02) | Identifies the engine actually used (EMB = EM-with-bootstrap) and that its posterior is a bootstrap approximation — the subject of the limitation note. | `derivation.qmd` §sec-combine limitation note ("the bootstrap-based EM sampler") | High | Same paper as the proposed row above; this is the *limitation-note* use. Acquire: BBT re-export → migrate → Mathpix convert → lit-sync → verify, then add `@honakerAmeliaIIProgram2011`. |
| **Tanner, M. A. & Wong, W. H. (1987).** "The Calculation of Posterior Distributions by Data Augmentation." *JASA* 82(398), 528–540. (with discussion) | **queued** (PDF in Zotero 2026-06-02) | Seminal data-augmentation source — the exact-posterior MCMC alternative to the bootstrap, alongside Schafer (1997, MVN application). | `derivation.qmd` §sec-combine limitation note ("proper data augmentation") | High | Likely citekey `tannerCalculationPosteriorDistributions1987` (confirm from BBT). Acquire: BBT re-export → migrate → Mathpix convert → lit-sync → verify, then add the cite. |
