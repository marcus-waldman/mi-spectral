...
| Proposed citation | Status | Claim it supports | Where it would land | Priority | Notes |
|---|---|---|---|---|---|
| **von Hippel, P. T. & Bartlett, J. W. (2021).** "Maximum likelihood multiple imputation: Faster imputations and consistent standard errors without posterior draws." *Statistical Science* 36(3), 400–420. | **proposed** | Names and develops the same engine split the DA audit found: PDMI (posterior draw MI) vs MLMI (maximum-likelihood / non-draw MI), with different higher-order behavior — the modern reference for MLE-centered imputation as a first-class construction | Engine-scope remark near @sec-qfun / §5 (W1 engine discussion); response to reviewer r1#8 | High | Proposed 2026-07-01 during the todo/040 cross-model derivation audit (Claude Fable 5). Training-weight knowledge; verify exact PDMI/MLMI definitions on read. |
| **Efron, B. (1979).** "Bootstrap methods: another look at the jackknife." *Annals of Statistics* 7(1), 1–26. (Or Efron & Tibshirani 1993, *An Introduction to the Bootstrap*, as the book form.) | **proposed** | Canonical citation for the nonparametric bootstrap and for bootstrap bias estimation ($\mathbb{E}^*[\tilde\theta]-\hat\theta$ estimates the sampling bias) — needed the moment the engine-scope remark calls Amelia's draw "a bootstrap" and states that $b_\Sigma^{\text{boot}}$ estimates $\mathrm{Bias}(\hat\Sigma_{\text{obs}})$. Note `efronAssessingAccuracyMaximum1978` in the corpus is Efron–Hinkley (observed vs expected information), a different paper. | Engine-scope remark near @sec-qfun; Q2 mechanism sentence in the r1#8 response | High (if the bootstrap mechanism sentence enters the manuscript; the complete-data second-moment identity itself is proved inline and needs no citation) | Proposed 2026-07-01 (todo/040 audit). |
...
| **von Hippel, P. T. (year uncertain — a Consensus.app search returned internally inconsistent metadata: title/byline "2013", DOI-embedded "2015", *Structural Equation Modeling* volume 23 implying ~2016; verify directly before Zotero entry).** "New confidence intervals and bias comparisons show that maximum likelihood can beat multiple imputation in small samples." *Structural Equation Modeling: A Multidisciplinary Journal*. | **proposed** | Single-authored companion to von Hippel & Bartlett (2021, row above): states the BIAS point directly rather than the SE/efficiency point — "both ML and MI can be biased, but the biases are larger under MI than under ML," with a stated mechanism (posterior draws inflate the imputed variable's variance). Possibly a tighter cite for the engine-scope bias claim than the 2021 paper alone. | Same landing as von Hippel-Bartlett 2021 — the engine-scope remark near @sec-qfun / the r1#8 response | Medium — nice-to-have alongside the 2021 paper, not a blocker | Surfaced 2026-07-02 via a Consensus.app due-diligence query Marcus ran before ratifying the todo/040 derivation.qmd amendment (query: "Does multiple imputation using Bayesian posterior draws differ in bias from maximum likelihood (non-draw) imputation?", saved as `a.txt`). Not yet independently confirmed to exist with these exact details — training-weight knowledge suggests the paper is real, but confirm venue/year/pages before acquiring. |
