...
| Proposed citation | Status | Claim it supports | Where it would land | Priority | Notes |
|---|---|---|---|---|---|
| **von Hippel, P. T. & Bartlett, J. W. (2021).** "Maximum likelihood multiple imputation: Faster imputations and consistent standard errors without posterior draws." *Statistical Science* 36(3), 400–420. | **cited** (`@vonhippelMaximumLikelihoodMultiple2021`, `manuscript/reviews/v1/response/r1-8-data-augmentation.md`, 2026-07-02) | Names and develops the same engine split the DA audit found: PDMI (posterior draw MI) vs MLMI (maximum-likelihood / non-draw MI), with different higher-order behavior — the modern reference for MLE-centered imputation as a first-class construction | Engine-scope remark near @sec-qfun / §5 (W1 engine discussion); response to reviewer r1#8 | High | Proposed 2026-07-01 (todo/040 audit, Claude Fable 5). Marcus added to Zotero + re-exported BBT 2026-07-02; PDF was in the Drive folder but misnamed (`von_maximum_likelihood_multiple_2021.pdf` — the auto-namer dropped "hippel" from the two-word surname), so `lit_sync.py`'s surname match missed it. Renamed to the citekey (`vonhippelMaximumLikelihoodMultiple2021.pdf`) for direct-match discovery; re-ran `/lit-sync`, Mathpix cache hit, markdown written. **Verified 2026-07-02** (`literature/vonhippelMaximumLikelihoodMultiple2021.md` read in full): Abstract (:25) and §1 Introduction (:43-45) name and define both terms exactly as claimed — "posterior draw multiple imputation (PDMI)" draws imputation parameters from the Bayesian posterior, "maximum likelihood multiple imputation (MLMI)" estimates them by ML/asymptotically-efficient alternatives. The behavioral difference goes beyond efficiency/SE: §1 (:47-48) states "In small samples, PDMI point estimates can also have more bias than MLMI point estimates, but the biases are trivial in moderate to large samples [33, 34]" — a genuine bias-not-just-variance claim (refs [33,34] = von Hippel 2016/2013a, the companion row below). §4-§5 formalize the higher-order divergence: $V_{\text{PDMI}}=V_{\text{ML}}+\frac{1}{M}V_{\text{ML}}\gamma_{\text{mis}}$ vs $V_{\text{MLMI}}=V_{\text{ML}}+\frac{1}{M}V_{\text{com}}\gamma_{\text{mis}}$ (eqs 4.4-4.5), and the WB variance estimators are genuinely different formulas under the two regimes (§5.1 vs §5.2), not a relabeling. Eligible for `@vonhippelMaximumLikelihoodMultiple2021`. |
| **Efron, B. (1979).** "Bootstrap methods: another look at the jackknife." *Annals of Statistics* 7(1), 1–26. (Or Efron & Tibshirani 1993, *An Introduction to the Bootstrap*, as the book form.) | **cited** (`@efronBootstrapMethodsAnother1979`, `manuscript/reviews/v1/response/r1-8-data-augmentation.md`, 2026-07-02) | Canonical citation for the nonparametric bootstrap and for bootstrap bias estimation ($\mathbb{E}^*[\tilde\theta]-\hat\theta$ estimates the sampling bias) — needed the moment the engine-scope remark calls Amelia's draw "a bootstrap" and states that $b_\Sigma^{\text{boot}}$ estimates $\mathrm{Bias}(\hat\Sigma_{\text{obs}})$. Note `efronAssessingAccuracyMaximum1978` in the corpus is Efron–Hinkley (observed vs expected information), a different paper. | Engine-scope remark near @sec-qfun; Q2 mechanism sentence in the r1#8 response | High (if the bootstrap mechanism sentence enters the manuscript; the complete-data second-moment identity itself is proved inline and needs no citation) | Proposed 2026-07-01 (todo/040 audit). Marcus added to Zotero + re-exported BBT 2026-07-02; PDF found + Mathpix-converted cleanly on the first `/lit-sync` pass. **Verified 2026-07-02** (`literature/efronBootstrapMethodsAnother1979.md` read in full): §2 (:66-137) is the canonical construction of the nonparametric bootstrap (resample $\hat F$ with replacement, approximate the sampling distribution of $R(\mathbf X,F)$ by the bootstrap distribution of $R^*=R(\mathbf X^*,\hat F)$) — confirms the "canonical source for the nonparametric bootstrap" half of the claim. The bias-estimation half is confirmed twice: §4 (:383, discriminant-analysis example) explicitly forms "the bias-corrected value based on the bootstrap," $\widehat{\text{error}}_F + E_*R^*$, i.e. uses $E_*[R^*]=E_*[\hat\theta^*-\hat\theta]$ as an estimate of the sampling bias of $\hat\theta$; §5 (:467, eq. 5.11-5.12) gives the general delta-method/infinitesimal-jackknife result $\text{Bias}_F\,\theta(\hat F)\approx\frac{1}{2n}\bar V$, derived from $E_*R(\mathbf P^*)\approx R(\mathbf e/n)+\frac{1}{2n}\bar V$ with $R(\mathbf e/n)=t(\mathbf X)-\theta(\hat F)=0$ — i.e. $E_*[\tilde\theta]-\hat\theta$ literally estimates the bias. Eligible for `@efronBootstrapMethodsAnother1979`. |
...
| **von Hippel, P. T. (2016).** "New confidence intervals and bias comparisons show that maximum likelihood can beat multiple imputation in small samples." *Structural Equation Modeling: A Multidisciplinary Journal* 23. DOI: 10.1080/10705511.2015.1047931. | **cited** (`@vonhippelNewConfidenceIntervals2016`, `manuscript/reviews/v1/response/r1-8-data-augmentation.md`, 2026-07-02) | Single-authored companion to von Hippel & Bartlett (2021, row above): states the BIAS point directly rather than the SE/efficiency point — "both ML and MI can be biased, but the biases are larger under MI than under ML," with a stated mechanism (posterior draws inflate the imputed variable's variance). Possibly a tighter cite for the engine-scope bias claim than the 2021 paper alone. | Same landing as von Hippel-Bartlett 2021 — the engine-scope remark near @sec-qfun / the r1#8 response | Medium — nice-to-have alongside the 2021 paper, not a blocker | Surfaced 2026-07-02 via a Consensus.app due-diligence query. **Year resolved to 2016** (the Zotero entry's DOI, `10.1080/10705511.2015.1047931`, confirms the SEM volume-23 guess; the "2013"/"2015" byline metadata was the Consensus.app inconsistency, not a real ambiguity). Marcus added to Zotero + re-exported BBT 2026-07-02; PDF was in the Drive folder but misnamed (same "hippel" dropped from the two-word surname as the 2021 row); renamed to citekey, re-ran `/lit-sync`, Mathpix cache hit. **Verified 2026-07-02** (`literature/vonhippelNewConfidenceIntervals2016.md` read in full): claim confirmed verbatim in the Introduction (:72), "We also confirm that, although both ML and MI can produce biased point estimates, the biases are larger under MI than they are under ML" — states the bias point directly, not just SE/efficiency (the efficiency point is a separate sentence at :70). The stated mechanism is confirmed too: :72 names "the variance of the imputed variable is overestimated" under MI as the pattern, and the "Posterior Draw Multiple Imputation" section (:370-417, esp. eq. at :412-413, $\Sigma_{\hat\alpha\hat\beta,\text{PD}}\approx 2\Sigma_{\hat\alpha\hat\beta,\text{ML}}$) derives it explicitly: posterior draws add their own sampling variability on top of the ML estimates' variability, roughly doubling it, which is the concrete mechanism by which PD-based imputation inflates variance/bias relative to MLMI. A closed-form bias expression for $\hat\sigma^2_{Y,\text{MLSI}}$ is also given (:363, eq. after "Bias"), consistent with the "closed-form expression for the bias" claimed in the abstract. Eligible for `@vonhippelNewConfidenceIntervals2016`. |
