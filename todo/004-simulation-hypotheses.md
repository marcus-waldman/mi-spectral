...
Joint MVN proper imputation via the data-augmentation algorithm of Schafer (1997, Algorithm 5.2), as implemented in `norm` / `mice` with `method = "norm"`. Prior on $(\mu, \Sigma)$ is the standard non-informative Jeffreys prior $p(\mu, \Sigma) \propto |\Sigma|^{-(p+1)/2}$. Burn-in = 200 sweeps; thinning between draws = 100 sweeps. Number of imputations $M$ specified per witness.
