...
# Each exhibit carries an explicit "perfect" reference (complete-data benchmark /
# nominal level / analytic target / y=x) and the achieved value, so the gap is
# visual.
#
# Legibility (todo/034 #3, 2026-06-27): every legend sits OUTSIDE the data area in
# the bottom margin; point and bar fills carry slight transparency; value labels
# are drawn with a white halo so they read through bars, bands, and reference
# lines; one coherent Okabe-Ito colourblind-safe palette throughout. The
# complete-data reference is labelled "complete-data benchmark" (T-08), not
# "oracle".
#
...
## ---- shared style -----------------------------------------------------------
# Okabe-Ito colourblind-safe palette, with fixed role -> colour mapping. Fills
# are drawn semi-transparent (A_FILL) so overplotted value labels read through.
COL <- list(
  benchmark   = "#111111",   # complete-data benchmark / nominal / analytic target / y=x
  perfect     = "#111111",   # alias used by reference-line helper
  oracle      = "#111111",   # retained internal colour key (== benchmark)
...
INK    = "#1A1A1A"           # near-black for value labels (always legible)
A_FILL = 0.72               # alpha for point / bar fills
A_BAND = 0.12               # alpha for reference bands
fillc <- function(col, a = A_FILL) { grDevices::adjustcolor(col, alpha.f = a) }
bandc <- function(col, a = A_BAND) { grDevices::adjustcolor(col, alpha.f = a) }

PAR_DEF = list(mar = c(4.4, 4.6, 3.0, 1.4), mgp = c(2.6, 0.7, 0), las = 1,
               cex.lab = 1.05, cex.axis = 0.95, cex.main = 1.12, font.main = 1)

png_open <- function(file, w_in, h_in, marb = NULL) {
  grDevices::png(file.path(FIGDIR, file), width = w_in, height = h_in,
                 units = "in", res = 300, type = "cairo")
  do.call(par, PAR_DEF)
  if (!is.null(marb)) { par(mar = c(marb, PAR_DEF$mar[2:4])) }
}

errbar <- function(x, lo, hi, col = INK, lwd = 2, wd = 0.04) {
  segments(x, lo, x, hi, col = col, lwd = lwd)
  segments(x - wd, lo, x + wd, lo, col = col, lwd = lwd)
  segments(x - wd, hi, x + wd, hi, col = col, lwd = lwd)
}

# Value/annotation text drawn with a white halo so it stays legible over bars,
# bands, and reference lines. Offsets are in user units, per-axis.
halo_text <- function(x, y, labels, col = INK, halo = "white", cex = 0.8,
                      font = 3, pos = NULL, offset = 0.5, hw = 0.22, n = 16) {
  ang = seq(0, 2 * pi, length.out = n + 1)[-(n + 1)]
  dx = hw * strwidth("o", cex = cex) * cos(ang)
  dy = hw * strheight("o", cex = cex) * sin(ang)
  for (i in seq_len(n)) {
    text(x + dx[i], y + dy[i], labels, col = halo, cex = cex, font = font,
         pos = pos, offset = offset, xpd = NA)
  }
  text(x, y, labels, col = col, cex = cex, font = font, pos = pos,
       offset = offset, xpd = NA)
}
note <- function(x, y, lab, col = INK, cex = 0.8, pos = 4, font = 3, offset = 0.5) {
  halo_text(x, y, lab, col = col, cex = cex, pos = pos, font = font, offset = offset)
}

ref_line <- function(y, lab, col = COL$perfect, lty = 2, at = NULL, cex = 0.85,
                     above = TRUE) {
  abline(h = y, col = col, lty = lty, lwd = 1.6)
  if (!is.null(lab)) {
    usr <- par("usr")
    xx = if (is.null(at)) { usr[1] + 0.015 * diff(usr[1:2]) } else { at }
    yy = y + (if (above) 1 else -1) * 0.018 * diff(usr[3:4])
    halo_text(xx, yy, lab, col = col, cex = cex, font = 3,
              pos = if (above) 3 else 1, offset = 0.15)
  }
}

# Legend placed in the bottom margin of the CURRENT figure/panel region, centred
# and outside the plot box (todo/034 #3). Works for single- and multi-panel.
margin_legend <- function(legend, ..., yoff = 0.010) {
  fig <- par("fig")
  xc = grconvertX((fig[1] + fig[2]) / 2, from = "ndc", to = "user")
  yb = grconvertY(fig[3] + yoff, from = "ndc", to = "user")
  legend(x = xc, y = yb, xjust = 0.5, yjust = 0, xpd = NA, bty = "n",
         legend = legend, ...)
}
...
draw_w1 <- function() {
  ...
  png_open("fig-w1-theorem.png", 6.8, 5.1, marb = 6.6)
  xx = seq_along(N)
...
draw_knownscale <- function() {
  ...
  png_open("fig-knownscale.png", 6.0, 5.0, marb = 6.2)
  xx = c(1, 2)
  ...
  segments(xx - 0.22, tgt, xx + 0.22, tgt, col = COL$benchmark, lwd = 2.4, lty = 1)
  note(xx[1], tgt[1], "perfect = 0", col = COL$benchmark, pos = 3, cex = 0.8)
  note(xx[2], tgt[2], sprintf("perfect = -1/2 tr(RIV) = %.3f", -half),
       col = COL$benchmark, pos = 1, cex = 0.8)
  # achieved
  errbar(xx, est - 1.96 * se, est + 1.96 * se, col = "#444444", lwd = 2, wd = 0.05)
  points(xx, est, pch = 23, bg = fillc(COL$corrected), col = INK, cex = 1.8)
  note(xx, est, sprintf("%.3f", est), col = INK, pos = 4, cex = 0.82)
  margin_legend(horiz = TRUE, cex = 0.82,
         legend = c("achieved E[T] (95% CI)", "analytic target (perfect)"),
         pch = c(23, NA), pt.bg = c(fillc(COL$corrected), NA),
         lty = c(NA, 1), col = c(INK, COL$benchmark), lwd = c(NA, 2.4))
  grDevices::dev.off()
...
draw_lrt <- function() {
  ...
  r0 = rt[rt$delta == 0, ]
  arms = c("benchmark", "corrected", "uncorrected", "Satorra-\nBentler")
  ...
  cols = c(COL$benchmark, COL$corrected, COL$uncorrected, COL$sb)
  png_open("fig-lrt-absorption.png", 6.6, 5.0, marb = 5.6)
  xx = seq_along(arms)
  ...
  rect(0.6, 0.05 - 0.005, length(arms) + 0.4, 0.05 + 0.005, col = bandc(COL$benchmark), border = NA)
  abline(h = 0.05, col = COL$benchmark, lty = 2, lwd = 1.6)
  note(0.7, 0.060, "nominal 0.05 (perfect)", col = COL$benchmark, cex = 0.82, pos = 4)
  for (i in seq_along(arms)) {
    if (!is.na(mcse[i])) { errbar(xx[i], vals[i] - mcse[i], vals[i] + mcse[i], col = "#444444", lwd = 2, wd = 0.06) }
  }
  points(xx, vals, pch = 21, bg = fillc(cols), col = INK, cex = 2)
  note(xx + 0.13, vals, sprintf("%.3f", vals), pos = 4, offset = 0.1, cex = 0.85, col = INK)
  note(2.5, 0.014, "corrected is slightly conservative (double-counting); uncorrected is\nabsorbed by the calibrated reference -- both near nominal",
       col = "#555555", cex = 0.72, pos = 3)
  grDevices::dev.off()
  cat("  fig-lrt-absorption.png  (Type I error: benchmark", vals[1], "/ corrected", vals[2], "/ uncorrected", vals[3], ")\n")
}
...
draw_discrimination <- function() {
  ...
  png_open("fig-discrimination.png", 6.6, 5.1, marb = 6.6)
  xx = seq_along(N)
  ...
  axis(1, at = xx, labels = N)
  abline(h = pred, col = COL$benchmark, lty = 1, lwd = 1.6)
  ...
  note(0.72, pred + 0.9, sprintf("projected trace  tr(RIV_perp) = %.2f  (correct, perfect)", pred),
       col = COL$benchmark, cex = 0.82, pos = 4)
  errbar(xx, est - 1.96 * se, est + 1.96 * se, col = "#444444", lwd = 2.2, wd = 0.05)
  points(xx, est, pch = 21, bg = fillc(COL$corrected), col = INK, cex = 1.7)
  note(xx, est, sprintf("%.2f", est), col = INK, pos = 4, cex = 0.82)
  ...
  margin_legend(horiz = TRUE, cex = 0.80,
         ...
         pch = c(21, NA, NA), pt.bg = c(fillc(COL$corrected), NA, NA),
         lty = c(NA, 1, 2), col = c(INK, COL$benchmark, COL$uncorrected), lwd = c(NA, 1.6, 1.6))
  grDevices::dev.off()
...
draw_pairing <- function() {
  ...
  png_open("fig-pairing.png", 6.6, 5.1, marb = 6.4)
  ...
  lines(N, single_h0, col = COL$uncorrected, lwd = 2.2); points(N, single_h0, pch = 16, col = fillc(COL$uncorrected, 0.85))
  lines(N, paired_fx, col = COL$sb, lwd = 2.0, lty = 2); points(N, paired_fx, pch = 17, col = fillc(COL$sb, 0.85))
  lines(N, paired_h0, col = COL$corrected, lwd = 2.4); points(N, paired_h0, pch = 16, col = fillc(COL$corrected, 0.85))
  ...
  margin_legend(horiz = TRUE, cex = 0.78,
...
# =============================================================================
# FIG 6 -- Selection sweep (W3). True-model selection rate: complete-data
# benchmark (perfect) vs uncorrected MI-AIC vs corrected, deterministic FIML,
# non-monotone MAR.
# =============================================================================
draw_w3 <- function() {
  ...
  s = s[order(s$N), ]
  N = s$N
  M = rbind(benchmark = s$pi_oracle, uncorrected = s$pi_uncorrected, corrected = s$pi_corrected)
  png_open("fig-w3-selection.png", 7.2, 5.1, marb = 6.0)
  cols = c(COL$benchmark, COL$uncorrected, COL$corrected)
  bp = barplot(M, beside = TRUE, names.arg = paste0("N=", N), col = fillc(cols), border = NA,
               ylim = c(0, 1.02), ylab = "True-model selection rate",
               main = "Selection: correction recovers toward the complete-data benchmark")
  for (j in seq_along(N)) {
    abline(h = M["benchmark", j], col = COL$benchmark, lty = 3, lwd = 1.2)
  }
  for (jj in seq_len(ncol(bp))) {
    for (ii in seq_len(nrow(bp))) {
      note(bp[ii, jj], M[ii, jj], sprintf("%.2f", M[ii, jj]), pos = 3, cex = 0.70, offset = 0.30, col = INK)
    }
  }
  note(mean(bp), 0.30, "100% of uncorrected misclassifications\nland on the largest-RIV candidate",
       col = "#555555", cex = 0.78, pos = 3)
  margin_legend(horiz = TRUE, cex = 0.82,
         legend = c("complete-data benchmark (perfect)", "uncorrected MI-AIC", "corrected MI-AIC"),
         fill = fillc(cols), border = NA)
  grDevices::dev.off()
...
# the complete-data benchmark per cell is the "perfect" reference line. Two-moment
# and equating reach the benchmark at rho=0.40; the weak-signal floor at rho=0.10
# is the failure.
# =============================================================================
draw_ladder <- function() {
  ...
  png_open("fig-ladder.png", 8.4, 5.5, marb = 7.4)
  bp = barplot(M, beside = TRUE, col = fillc(cols), border = NA, ylim = c(0, 1.02),
               names.arg = rep("", 3), ylab = "True-model selection rate",
               main = "Distribution-matching ladder: arms vs the complete-data benchmark")
  ...
  for (j in 1:3) {
    segments(bp[1, j] - 0.5, L$pi_oracle[j], bp[4, j] + 0.5, L$pi_oracle[j],
             col = COL$benchmark, lty = 2, lwd = 1.8)
    halo_text(bp[1, j] - 0.5, L$pi_oracle[j], sprintf("benchmark %.2f", L$pi_oracle[j]),
              col = COL$benchmark, cex = 0.68, font = 3, pos = 3, offset = 0.25)
  }
  for (jj in seq_len(ncol(bp))) {
    for (ii in seq_len(nrow(bp))) {
      note(bp[ii, jj], M[ii, jj], sprintf("%.2f", M[ii, jj]), pos = 3, cex = 0.60, offset = 0.22, col = INK)
    }
  }
  note(grp_centre[2], 0.745, "weak-signal floor:", col = COL$uncorrected, cex = 0.74, pos = 3)
  note(grp_centre[2], 0.745, "no arm reaches the benchmark", col = COL$uncorrected, cex = 0.74, pos = 1)
  margin_legend(ncol = 3, cex = 0.80,
         legend = c(arms, "complete-data benchmark (perfect)"),
         fill = c(fillc(cols), NA), border = NA, lty = c(NA, NA, NA, NA, 2),
         col = c(NA, NA, NA, NA, COL$benchmark), lwd = c(NA, NA, NA, NA, 1.8))
  grDevices::dev.off()
  cat("  fig-ladder.png\n")
}
...
draw_ladder_internals <- function() {
  L <- read.csv("verification/cache/ic-ladder-summary.csv")
  m = L[L$cell == "MAIN", ]
  png_open("fig-ladder-internals.png", 9.6, 4.9)
  par(mfrow = c(1, 2), mar = c(4.6, 4.6, 3.0, 1.4))
  ...
  cols = c(COL$uncorrected, COL$mean, COL$twomoment, COL$equating)
  bp = barplot(ks, names.arg = arms, col = fillc(cols), border = NA, ylim = c(0, 0.33),
               ylab = "KS distance to complete-data null", main = "A. Null side calibrates")
  note(bp, ks, sprintf("%.3f", ks), pos = 3, cex = 0.78, offset = 0.5, col = INK)
  ref_line(0, "perfect = 0", col = COL$benchmark, lty = 2, at = bp[1], above = TRUE)
  ...
  abline(0, 1, col = COL$benchmark, lty = 2, lwd = 1.8)
  note(mean(lim), max(lim), "y = x (perfect)", col = COL$benchmark, cex = 0.82, pos = 1)
  points(pred, meas, pch = c(21, 23), bg = fillc(c(COL$twomoment, COL$equating)), col = INK, cex = 1.9)
  note(pred, meas, c(sprintf("two-moment (a_k=%.2f)", pred[1]),
...
draw_w4 <- function() {
  ...
  png_open("fig-w4-nonnested.png", 9.8, 5.4)
  par(mfrow = c(1, 2), mar = c(7.6, 4.6, 3.0, 1.2))
  ...
  bp = barplot(M, beside = TRUE, col = fillc(c(COL$mar, COL$mcar)), border = NA, ylim = c(0, 3.3),
               ylab = "(A)+(C) level (deviance units)", main = "A. Level collapses under MCAR")
  for (jj in seq_len(ncol(bp))) {
    for (ii in seq_len(nrow(bp))) {
      note(bp[ii, jj], t(M)[ii, jj], sprintf("%.2f", t(M)[ii, jj]), pos = 3, cex = 0.74, offset = 0.40, col = INK)
    }
  }
  ref_line(0, "MCAR target = 0 (perfect)", col = COL$benchmark, lty = 2, at = bp[1], above = TRUE)
  margin_legend(horiz = TRUE, cex = 0.82, fill = fillc(c(COL$mar, COL$mcar)), border = NA,
